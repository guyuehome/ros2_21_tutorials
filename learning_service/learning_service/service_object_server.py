...
class ImageSubscriber(Node):
  def __init__(self, name):
    ...
    self.srv = self.create_service(GetObjectPosition, 'get_target_position', self.object_position_callback)     # 创建服务器对象（接口类型、服务名、服务器回调函数）
  ...
  def object_position_callback(self, request, response):             # 创建回调函数，执行收到请求后对数据的处理
    if request.get == True:
        ...
        self.get_logger().info('Object position\nx: %d y: %d' % (response.x, response.y))   # 输出日志信息，提示已经反馈
    else:
        response.x = 0
        response.y = 0
